    ...
    return (
        <div className='bg-white p-4 col-start-4 col-end-13 text-black'>
            ...
            <div className='flex space-x-4 px-3'>
                {elementDatas.map((d, idx) => <button onClick={() => setCurrentElement(d)} key={idx} className={`text-[#AACDED] hover:cursor-pointer ${currentElement === d && "text-[#27639C] border-b-4 py-2 border-[#23619C]"}`}>{d}</button>)}
            </div>
